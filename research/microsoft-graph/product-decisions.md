...
|---|---|---|---|
| OWN-00 | The native companion is pure Rust with the accepted native UI toolkit. System-browser authorization code with PKCE is the MVP identity path; WAM is deferred. The Office.js/TypeScript add-in is the supported sandbox exception. | ADR-001, ADR-002, ADR-014 | G-ID, G-ADDIN |
...
| OWN-06 | Minimized encrypted local state is the MVP baseline, including native UI handling of secrets. Microsoft-hosted/cross-device state is a later feasibility adapter. Secure persistence failure is session-only or fail-closed. | ADR-PRIV-001, ADR-005, ADR-014 | G-STATE, G-PRIV, G-SEC-AUDIT |
