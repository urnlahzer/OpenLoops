...
impl std::fmt::Display for ProviderError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::InvalidKey => "Enter a valid Ollama API key.",
            Self::Unauthorized => "Ollama rejected the API key or account access.",
            Self::InputUnavailable => "Could not read a model selection from the terminal.",
            Self::Timeout => "Ollama did not finish within 60 seconds. Try the model again or select another model.",
            Self::Network => "Could not establish or complete a secure connection to Ollama Cloud. Check connectivity.",
            Self::RateLimited => "Ollama returned HTTP 429 (rate limit). Wait before trying again.",
            Self::Quota => "Ollama returned HTTP 402. Check your Ollama Cloud plan or usage balance.",
            Self::RequestRejected(status) => return write!(f, "Ollama rejected the request (HTTP {status}). Check that the selected model supports cloud chat."),
            Self::ServerError(status) => return write!(f, "Ollama returned a server error (HTTP {status}). Try again later or choose another model."),
            Self::ModelUnavailable => "The selected model is not in Ollama Cloud's model list.",
            Self::InvalidResponse => "Ollama returned an incomplete or unsupported response.",
            Self::InputTooLarge => "The selected message projection exceeds the analysis limits.",
            Self::ResponseTooLarge => "Ollama's response exceeds the allowed size.",
