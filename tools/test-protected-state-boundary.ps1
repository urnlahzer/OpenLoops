...
function Invoke-ExactWorkspaceCase([string]$Name, [string]$Expected, [scriptblock]$Mutate) {
    ...
    $inputs = @('Cargo.lock','Cargo.toml','crates/openloops-application/Cargo.toml','crates/openloops-application/src/lib.rs','crates/openloops-contracts/Cargo.toml','crates/openloops-contracts/src/lib.rs','crates/openloops-desktop/Cargo.toml','crates/openloops-desktop/src/main.rs','crates/openloops-domain/Cargo.toml','crates/openloops-domain/src/lib.rs','crates/openloops-graph/Cargo.toml','crates/openloops-graph/src/lib.rs','crates/openloops-inference/Cargo.toml','crates/openloops-inference/src/lib.rs','crates/openloops-persistence/Cargo.toml','crates/openloops-persistence/src/lib.rs','rust-toolchain.toml')
...
try {
    ...
    Invoke-ExactWorkspaceCase 'dependency alias in another crate' 'P0-STATE-CLAIMS-001' { param($root) $path=Join-Path $root 'crates\openloops-persistence\Cargo.toml';$raw=Get-Content -Raw $path;$raw=$raw.Replace('[dependencies]',"[dependencies]`ndatabase_adapter = { package = `"rusqlite`", version = `"0.0.0`" }");Set-Content -LiteralPath $path -Value $raw -Encoding utf8NoBOM -NoNewline }
    Invoke-ExactWorkspaceCase 'transitive selected dependency in Cargo lock' 'P0-STATE-CLAIMS-001' { param($root) Add-Content -LiteralPath (Join-Path $root 'Cargo.lock') -Encoding utf8NoBOM -Value "`n[[package]]`nname = `"rusqlite`"`nversion = `"0.0.0`"" }
