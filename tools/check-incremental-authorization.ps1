...
# Re-scoped on 2026-09-14 from the Phase 0 zero-runtime scan without removing
# any pattern. Network dependencies are confined to openloops-graph's optional
# live-connection dependencies, plus optional reqwest behind both inference provider
# features; TcpStream and HttpClient stay in the reviewed graph implementation paths.
$graphCargo = [IO.Path]::GetFullPath((Join-Path $repoRoot 'crates/openloops-graph/Cargo.toml'))
$graphCargoText = Get-Content -Raw -LiteralPath $graphCargo
$liveFeature = [regex]::Match($graphCargoText, '(?m)^\s*live-connection\s*=\s*\[([^\]]+)\]')
$inferenceCargo = [IO.Path]::GetFullPath((Join-Path $repoRoot 'crates/openloops-inference/Cargo.toml'))
$inferenceCargoText = Get-Content -Raw -LiteralPath $inferenceCargo
$ollamaCloudFeature = [regex]::Match($inferenceCargoText, '(?m)^\s*ollama-cloud\s*=\s*\[([^\]]+)\]')
$openrouterFeature = [regex]::Match($inferenceCargoText, '(?m)^\s*openrouter\s*=\s*\[([^\]]+)\]')
foreach ($cargoFile in $cargoFiles) {
    $cargoText = Get-Content -Raw -LiteralPath $cargoFile.FullName
    foreach ($dependencyMatch in [regex]::Matches($cargoText, '(?im)^\s*(reqwest|oauth2|hyper|ureq|tokio)\s*=\s*(.+)$')) {
        $dependencyName = $dependencyMatch.Groups[1].Value
        $dependencyDeclaration = $dependencyMatch.Groups[2].Value
        $sanctionedGraphDependency = $cargoFile.FullName -eq $graphCargo -and $liveFeature.Success -and ([regex]::Matches($liveFeature.Groups[1].Value, ('"dep:' + [regex]::Escape($dependencyName) + '"'))).Count -eq 1
        $sanctionedInferenceReqwest = $dependencyName -eq 'reqwest' -and $cargoFile.FullName -eq $inferenceCargo -and $ollamaCloudFeature.Success -and $openrouterFeature.Success -and ([regex]::Matches($ollamaCloudFeature.Groups[1].Value, '"dep:reqwest"')).Count -eq 1 -and ([regex]::Matches($openrouterFeature.Groups[1].Value, '"dep:reqwest"')).Count -eq 1
        if ($dependencyDeclaration -notmatch '\boptional\s*=\s*true\b' -or (-not $sanctionedGraphDependency -and -not $sanctionedInferenceReqwest)) { Add-Failure 'P0-AUTHZ-CROSS-CONTRACT-001' }
    }
}
$rustSourceFiles = @(Get-ChildItem -LiteralPath (Join-Path $repoRoot 'crates') -Recurse -File -Include '*.rs')
foreach ($sourceFile in $rustSourceFiles) {
    $sourceText = Get-Content -Raw -LiteralPath $sourceFile.FullName
    if ($sourceText -notmatch '(?i)\b(TcpStream|HttpClient)\b') { continue }
    $relative = [IO.Path]::GetRelativePath($repoRoot, $sourceFile.FullName).Replace('\', '/')
    $sanctioned = $relative -eq 'crates/openloops-graph/src/callback.rs' -or $relative -eq 'crates/openloops-graph/src/live.rs' -or $relative -like 'crates/openloops-graph/src/live/*'
    if (-not $sanctioned) { Add-Failure 'P0-AUTHZ-CROSS-CONTRACT-001' }
}
$addinSourceFiles = @(Get-ChildItem -LiteralPath (Join-Path $repoRoot 'outlook-addin') -Recurse -File -Include '*.ts','*.tsx','*.js' | Where-Object { $_.FullName -notmatch '[\\/]\.generated[\\/]' })
if ($addinSourceFiles.Count -gt 0 -and (Select-String -LiteralPath @($addinSourceFiles.FullName) -Pattern '(?i)\bXMLHttpRequest\b|\bfetch\s*\(' -Quiet -ErrorAction SilentlyContinue)) { Add-Failure 'P0-AUTHZ-CROSS-CONTRACT-001' }
