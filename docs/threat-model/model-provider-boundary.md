...
| Oversized, slow, or unbounded streaming response exhausts resources | Use non-streaming requests, exact request/response byte limits, a five-second connect limit, a 60-second per-read idle-guard limit (covering the header wait and any single silent read), and an independent 150-second total wall-time bound on the whole request -- from `send()` to the last body byte -- checked at least every 250 ms from a background thread so it is enforced even while the connection withholds response headers entirely (Ollama Cloud does this until generation finishes) or stays completely silent mid-body. A bounded response reader cancels immediately over a byte/time cap. |
| Stop does not abort a request already in flight | The same 250 ms poll that enforces the wall-time bound, in both the `send()` and body-read phases, also checks a caller-supplied cancel flag, so clicking Stop aborts the in-flight request (`ProviderError::Cancelled`) within about a second regardless of which phase it is in, not only between conversations. Abandoning it this way does not instantly free the background thread or its socket, which can still linger for up to the 60-second per-read idle-guard limit above. |
