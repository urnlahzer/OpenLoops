...
| Oversized, slow, or unbounded streaming response exhausts resources | Use non-streaming requests, exact request/response byte limits, a five-second connect limit, a 60-second per-read idle-guard limit (covering the header wait and any single silent read), and an independent 150-second total wall-time bound on the whole request, checked at least every 250 ms from a background reader thread so it is enforced even while the connection stays completely silent. A bounded response reader cancels immediately over a byte/time cap. |
| Stop does not abort a request already in flight | The same 250 ms poll that enforces the wall-time bound also checks a caller-supplied cancel flag, so clicking Stop aborts the in-flight request (`ProviderError::Cancelled`) within about a second rather than only between conversations. |
