...
| P0-AUTH-DEPENDENCIES-001 | Reviewed direct crate versions/features/licenses are exact and activated only in `openloops-graph` behind `live-connection`, plus `reqwest` in `openloops-inference` behind its provider features; the lock must contain exactly the contract pins, while Tokio remains transitive-only. Git/wildcard dependencies and ad hoc OAuth/crypto are prohibited. | Authentication manifest; Cargo workspace/lock; checker | Executable confinement boundary | Identity implementation and G-SEC-AUDIT |
...
| P0-SYNC-CROSS-CONTRACT-001 | ADR-004 reconciles exactly with governance, ADR-003 permissions/folders, account binding, ADR-PRIV-001 fields/lifecycle, support/build inactivity, deferred ADR ownership, threat routing, and current Microsoft source boundaries; network dependencies and symbols are confined to the reviewed `openloops-graph` feature and source paths. | All Phase 0 contracts; deterministic checker | Executable confinement boundary | Next Phase 0 work item |
...
| P0-MODEL-SOURCES-001 | The dated source inventory pins current official Ollama local/cloud bases, direct API-key authentication, local cloud-disable boundary, model listing, API versioning caveat, the current Cloud structured-output limitation, TypeSafe's typed Jev API, and OpenRouter's decisions endpoint. Application validation never relies on a provider-side schema guarantee. | Model/support manifests; official Ollama, TypeSafe, and OpenRouter documentation | Dated executable research contract | Source freshness review |
| P0-MODEL-DECISIONS-001 | The Jev registry pins schema version 1 and `typesafe/jev-1.13`; every question has a supported typed answer and ordered review thresholds, and choice questions have 1–255 issued options. The registry hash and OpenRouter decision request/response boundary are pinned. | Decision registry; model manifest; TypeSafe and OpenRouter API documentation | Executable decision-model contract | G-MODEL, G-PRIV |
